...
public class PatternView extends View
{
    public static class Cell implements Parcelable
    {
        ...
        /**
         * Gets the ID.It is counted from left to icon_right, top to bottom of the matrix, starting by zero.
         *
         * @return the ID.
         */
        public int getId()
    ...
    /**
     * How many milliseconds we spend animating each circle_white of a lock pattern if the animating mode is set. The entire
    ...
    public PatternView(Context context, AttributeSet attrs)
    {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.PatternView);
        mRegularColor = typedArray.getColor(R.styleable.PatternView_regularColor, Color.WHITE);
        mErrorColor = typedArray.getColor(R.styleable.PatternView_errorColor,Color.RED);
        mSuccessColor = typedArray.getColor(R.styleable.PatternView_successColor,Color.GREEN);
        typedArray.recycle();
    ...
    public void setPatternDotColor(String color)
    {
        int dotColor = Color.parseColor(color);

        mRegularColor = dotColor;
        mErrorColor = dotColor;
        mSuccessColor = dotColor;

        mPathPaint.setColor(dotColor);
    ...
    private void startSizeAnimation(float start, float end, long duration, Interpolator interpolator, final CellState state, final Runnable endRunnable)
    {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB)
        {
            ...
            {
            ValueAnimator valueAnimator = ValueAnimator.ofFloat(start, end);valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener()
                    {
                        @Override
                        public void onAnimationUpdate(ValueAnimator animation)
                        {
    ...
    /**
     * @param partOfPattern Whether this circle_white is part of the pattern.
     */
